...
func (s *apiServer) ListFiles(ctx context.Context, request gen.ListFilesRequestObject) (gen.ListFilesResponseObject, error) {
	...
	out := make([]models.FileMeta, 0, len(metas))
	for _, m := range metas {
		out = append(out, models.FileMeta{Path: m.Path, Sha: m.SHA, Size: m.Size})
	}
...
func (s *apiServer) ReadFile(ctx context.Context, request gen.ReadFileRequestObject) (gen.ReadFileResponseObject, error) {
	...
	return gen.ReadFile200JSONResponse(models.FileContent{
		Path:    fc.Path,
		Content: fc.Content,
		Sha:     fc.SHA,
	}), nil
...
func applyRequestFromWire(in models.ApplyRequest) files.ApplyRequest {
...
func applyResultToWire(res *files.ApplyResult) models.ApplyResult {
	out := models.ApplyResult{CommitSha: res.CommitSHA, Files: make([]models.FileMeta, 0, len(res.Files))}
	for _, f := range res.Files {
		out.Files = append(out.Files, models.FileMeta{Path: f.Path, Sha: f.SHA, Size: f.Size})
	}
	for _, w := range res.Warnings {
		out.Warnings = append(out.Warnings, models.Warning{Path: w.Path, Code: w.Code, Message: w.Message})
	}
	return out
}
