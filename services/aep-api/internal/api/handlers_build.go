...
func (s *apiServer) BuildProject(ctx context.Context, request gen.BuildProjectRequestObject) (gen.BuildProjectResponseObject, error) {
	...
	if len(failures) > 0 {
		return gen.BuildProject200JSONResponse(models.BuildResponse{Failures: toInputFailures(failures)}), nil
	}
	return gen.BuildProject200JSONResponse(models.BuildResponse{Tag: tag}), nil
}
...
func toBuildInputItems(in []models.BuildInputItem) []build.BuildInputItem {
	if len(in) == 0 {
		return nil
	}
...
func toInputFailures(in []build.InputFailure) []models.InputFailure {
	out := make([]models.InputFailure, 0, len(in))
	for _, f := range in {
		out = append(out, models.InputFailure{
...
func toBuildStatus(st build.BuildStatus) models.BuildStatus {
	var tasks []models.BuildStatusTask
	for _, t := range st.Tasks {
		tasks = append(tasks, models.BuildStatusTask{
			IssueNumber: t.IssueNumber,
			Title:       t.Title,
			Status:      models.BuildStatusTaskStatus(t.Status),
		})
	}
	return models.BuildStatus{
		Status:         st.Status,
...
func toBuildList(l build.BuildList) models.BuildList {
	// Builds stays non-nil so the JSON body is [] rather than null.
	builds := make([]models.BuildSummary, 0, len(l.Builds))
	for _, b := range l.Builds {
		s := models.BuildSummary{
			Tag:       b.Tag,
			Status:    models.BuildSummaryStatus(b.Status),
			Reason:    b.Reason,
			StartedAt: b.StartedAt,
			Tasks: models.BuildTally{
				Total:  b.Tasks.Total,
				Done:   b.Tasks.Done,
...
func toBuildPreflight(pf build.BuildPreflight) models.BuildPreflight {
	// Items stays non-nil so the JSON body is [] rather than null.
	items := make([]models.PreflightItem, 0, len(pf.Items))
	for _, it := range pf.Items {
		var cfg []models.ConfigKeyView
		for _, k := range it.Config {
			cfg = append(cfg, models.ConfigKeyView{
				Key:          k.Key,
				Secret:       k.Secret,
		...
		items = append(items, models.PreflightItem{
			Component:    it.Component,
			Dependency:   it.Dependency,
			Kind:         models.PreflightItemKind(it.Kind),
	...
	return models.BuildPreflight{NeedsInput: pf.NeedsInput, Items: items}
}
