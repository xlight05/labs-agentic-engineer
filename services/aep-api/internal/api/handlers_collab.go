...
func (s *apiServer) GetSpecCollabSession(ctx context.Context, request gen.GetSpecCollabSessionRequestObject) (gen.GetSpecCollabSessionResponseObject, error) {
	...
	name, email := requirements.ParseDisplayIdentity(headerValue(request.Params.Authorization))
	return gen.GetSpecCollabSession200JSONResponse(models.CollabSessionOutputBody{
...
func (s *apiServer) ValidateCollabAccess(ctx context.Context, request gen.ValidateCollabAccessRequestObject) (gen.ValidateCollabAccessResponseObject, error) {
	...
	roomID := strings.TrimSpace(headerValue(request.Params.XRoomID))
	if roomID == "" {
	...
	name, email := requirements.ParseDisplayIdentity(headerValue(request.Params.Authorization))
	return gen.ValidateCollabAccess200JSONResponse(models.CollabValidateOutputBody{
		Name:        name,
		Email:       email,
		ProjectName: project,
	}), nil
...
// headerValue unwraps an optional generated header param ("" when absent).
